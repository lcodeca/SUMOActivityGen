...
class GenerateTAZandWeightsFromOSM:
    ...
    def save_taz_to_osm(self, filename):
        ...
        print(f"Plotting TAZ to OpenStreetMap in file {filename}.")
        colors = [
            "#0000FF",
            "#0040FF",
            "#0080FF",
    ...
    def _filter_boundaries_from_osm(self):
        ...
        for way in tqdm(self._osm["way"]):
            if way["id"] in self._osm_boundaries["way"]:
                self._osm_boundaries["way"][way["id"]] = way
                for ndid in way["nd"]:
                    self._osm_boundaries["node"][ndid["ref"]] = {}
        for node in tqdm(self._osm["node"]):
            if node["id"] in self._osm_boundaries["node"]:
                self._osm_boundaries["node"][node["id"]] = node
        print(
            f"Found {len(self._osm_boundaries['relation'])} administrative boundaries."
        )
    ...
    def _build_taz_from_osm(self):
        ...
        if not self._param.single_taz:
            for id_boundary, boundary in tqdm(self._osm_boundaries["relation"].items()):
                ...
                if not boundary:
                    print(f"Empty boundary {id_boundary}")
                    continue
                ...
                if len(list_of_nodes) <= 2:
                    print(f"Boundary {id_boundary} has {len(list_of_nodes)} nodes.")
                    continue
                try:
                    ...
                    print(
                        f"Impossible to create the convex hull for boundary {id_boundary}."
                    )
                    continue
                # ----------------------------------------------------------------------------------
            ...
            print(f"Generated {len(self._taz)} TAZ from OSM boundaries.")
    ...
    def _edges_filter(self):
        ...
        for edge in tqdm(self._net.getEdges()):
            if self._param.single_taz:
                ...
                for coord in edge.getShape():
                    ...
                    for id_taz in self._taz:  # pylint: disable=C0206
        ...
        print(f"Saved {len(self._taz)} TAZ after the edges filtering.")
    ...
    def _filter_buildings_from_osm(self):
        ...
        for way in tqdm(self._osm["way"]):
            ...
            self._osm_buildings[way["id"]]["nodes"] = []
            for ndid in way["nd"]:
                self._osm_buildings[way["id"]]["nodes"].append(nodes[ndid["ref"]])
        print(f"Found {len(self._osm_buildings)} buildings.")
    ...
    def _processing_buildings(self):
        ...
        for building in self._osm_buildings.values():
            ...
            building["tag"].append(
                {"k": "centroid", "v": f"{centroid[0]}, {centroid[1]}"}
            )
            ...
            while pedestrian_edge_info is None or generic_edge_info is None:
                ...
                if radius > parameters["max_entrance_dist"]:
                    ...
                    if (
                        ...
                        print(
                            f"Warning: pedestrian edge {pedestrian_edge_info.getID()} is outside the TAZ. "
                            f"[distance: {pedestrian_dist_edge_oth_taz}m]"
                        )
                    if (
                        ...
                        print(
                            f"Warning: passenger edge {generic_edge_info.getID()} is outside the TAZ. "
                            f"[distance: {generic_dist_edge_oth_taz}m]"
                        )
                radius *= 2

            if generic_edge_info and generic_dist_edge > 500.0:
                print(
                    f"A building entrance {generic_edge_info.getID()} [passenger] is {generic_dist_edge} meters away."
                )
            if pedestrian_edge_info and pedestrian_dist_edge > 500.0:
                print(
                    f"A building entrance {pedestrian_edge_info.getID()} [pedestrian] is {pedestrian_dist_edge} meters away."
                )
    ...
    def _write_poly_files(self, prefix):
        ...
        for value in self._taz.values():
            if not value["buildings"]:
                continue
            filename = f"{prefix}.{value['ref']}.csv"
            with open(filename, "w") as csvfile:  # pylint: disable=W1514
